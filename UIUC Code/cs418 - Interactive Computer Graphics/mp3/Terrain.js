...
//                      Math.ceil(Math.random() * (this.maxY-this.minY)),0);
     ...
     */
    calculateNormals() {
      var normals = new Array(3*this.numVertices).fill(0);
      for(var i = 0; i < this.numFaces; i++){
        var face = [];
        face[0] = this.faceData[3*i+0];
        face[1] = this.faceData[3*i+1];
        face[2] = this.faceData[3*i+2];
        //var firstVertex = this.faceData[3*i];
        //var secondVertex = this.faceData[3*i+1];
        //var thirdVertes = this.faceData[3*i+2];
        var vertices = [];

        vertices[0] = [this.positionData[face[0]*3], this.positionData[face[0]*3+1],this.positionData[face[0]*3+2]];
        vertices[1] = [this.positionData[face[1]*3], this.positionData[face[1]*3+1],this.positionData[face[1]*3+2]];
        vertices[2] = [this.positionData[face[2]*3], this.positionData[face[2]*3+1],this.positionData[face[2]*3+2]];
          //this for loop gets the three vertices in a triangle for later work


        //var normals = [[0,0,0],[0,0,0],[0,0,0]];
        //this for loop accesses the normal vectors that are in place at the moment

        var cross1 = glMatrix.vec3.fromValues(vertices[1][0] - vertices[0][0], vertices[1][1] - vertices[0][1], vertices[1][2] - vertices[0][2]);
        var cross2 = glMatrix.vec3.fromValues(vertices[2][0] - vertices[0][0], vertices[2][1] - vertices[0][1], vertices[2][2] - vertices[0][2]);
        //for(var j=0;j<3;j++){
        /*for(var j=0;j<3;j++){
...
*/
        ...
        //cross product
        for(var j=0;j<3;j++){
          normals[face[j]*3] += faceNorm[0];
          normals[face[j]*3+1] += faceNorm[1];
          normals[face[j]*3+2] += faceNorm[2];
        }
        //[this.normalData[face[0]*3], this.normalData[face[0]*3+1], this.normalData[face[0]*3+2]] = normals[0];
        //[this.normalData[face[1]*3], this.normalData[face[1]*3+1], this.normalData[face[1]*3+2]] = normals[1];
        //[this.normalData[face[2]*3], this.normalData[face[2]*3+1], this.normalData[face[2]*3+2]] = normals[2];
      ...
      for(var i = 0; i <= this.numVertices; i++){
        ...
        var vertex = [normals[i*3], normals[i*3+1], normals[i*3+2]];
        var normal = glMatrix.vec3.create();
        glMatrix.vec3.normalize(normal,vertex);//add another vec3 in case
        [this.normalData[i*3], this.normalData[i*3+1], this.normalData[i*3+2]] = normal;
      }
