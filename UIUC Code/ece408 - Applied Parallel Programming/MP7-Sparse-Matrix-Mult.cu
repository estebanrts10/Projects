...
__global__ void spmvJDSKernel(float *out, int *matColStart, int *matCols,
  ...
  if(row < dim){
    float dot = 0;
    unsigned int sec = 0;
    while (/*matColStart[sec+1] - matColStart[sec] > row && */sec<matRows[row]){
    ...
    for(int i = 0; i < matRows[row]; i++){
      dot += matData[row+i*dim] * vec[matCols[row+dim*i]];
      //out[matRows[i]] += matData[i] * vec[matCols[i]];
    }
    __syncthreads();
    out[matRowPerm[row]] = dot;*/
  }
...
static void spmvJDS(float *out, int *matColStart, int *matCols,
  ...
  dim3 gridDim(ceil(dim*1.0 / 256), 1, 1);
