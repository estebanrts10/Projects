/*MP11 implements a floorplan through a binary tree by using modules
at leaves and cutlines at internal nodes. There are many functions
that swap nodes, swap boundaries, and tests for leave or internal
nodes. The function init_slicing_tree generates the tree, get_total_resources
finds the sum of the ALUs, registers, etc. that the entire floorplan needs,
postfix_traversal goes through the tree in a postfix manner, and the swap
functions swaps the module, parents, nodes, and children. */
//Partner: ry6
...
int is_leaf_node(node_t* ptr) {
  ...
  if (ptr->left==NULL && ptr->right==NULL){
    return 1;     //if the node is a leaf(no children) return 1
  }
  return 0;
}
...
int is_internal_node(node_t* ptr) {
  if (ptr->left!=NULL && ptr->right!=NULL){
    return 1;   //if the node is not a leaf(has children) return 1
  }
...
// Return 1 if the given subtree rooted at node 'b' resides in the subtree rooted at node 'a'.
  ...
  if(a==b){   //if both nodes are equal then return 1
  return 1;
}
  else if(is_internal_node(a)==1){//keeps going until children are null
   //keep checking the left and right childrens of given node
  if(is_in_subtree(a->left, b)==1||is_in_subtree(a->right,b)==1){//recursive call with both children
  return 1;   //
  }
  }
   return 0;
...
void rotate(node_t* ptr) {
   int temp;
    temp=(ptr->module)->w;//makes h = to w and w = to h
...
void recut(node_t* ptr) {
  ...
  if(ptr->cutline==V){  //if its vertical it makes it horizontal
    ptr->cutline=H;
  }
  else if(ptr->cutline==H){ //if its horizontal it makes it vertical
    ptr->cutline=V;
  }
...
void swap_module(node_t* a, node_t* b) {
  ...
  module_t *temp=a->module; //places a module into temp
  a->module=b->module;      //b module into a module
  b->module=temp;           //places temp(a) into module b
...
void swap_topology(node_t* a, node_t* b) {
  ...
  node_t* temp_P=a->parent; //

  if(a->parent->left==a){   //check if a is on the left
    a->parent->left=b;      //tranformations for a
    a->parent = b->parent;
    if(b->parent->left== b){//check if b is on the left
      b->parent->left = a;  //tranformation for b
      b->parent = temp_P;
    }
    else if(b->parent->right== b){  //check if b is on the right
      b->parent->right = a; //transformation for b
      b->parent = temp_P;
    }
  }

  else if(a->parent->right==a){   //check if a is on the right
    a->parent->right=b;       //transformation for a
    a->parent=b->parent;
    if(b->parent->left== b){  //check if b is on the left
      b->parent->left = a;    //tranformation for b
      b->parent = temp_P;
      ...
      b->parent->right = a;   //tranformation for b
      b->parent = temp_P;
    }
  }
...
//if the module is not null you set it to the module in ptr
...
*nth=*nth+1;    //add one to the nth pointer
...
{
  int temp;
  temp=ptr->module->resource;//create a temp variable for resource
  if (ptr->left!=NULL && ptr->right!=NULL){ //if both left and are not null
  temp=get_total_resource(ptr->left)+get_total_resource(ptr->right);
}//then do recursion to add the value on the left and the value of the right
else{
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  if(n==num_modules-1){
  ptr->module=&modules[n];  //base case sets modules into module of ptr
  ptr->cutline=UNDEFINED_CUTLINE;
  ptr->parent=par;    //the parent of ptr is par
  ptr->left=NULL;     //it is a leaf node so it has no right or left node
  ptr->right=NULL;

  return ptr;   //return pointer after base case to end function
  }
  ptr->parent = par;    //par is the parent of ptr
  ptr->cutline=V;       //the cutline of ptr is vertical


  node_t * ptr_R =(node_t*) malloc(sizeof(node_t));
  ptr_R->parent=ptr;    //parent of the right node is ptr
  ptr_R->cutline=UNDEFINED_CUTLINE;
  ptr->right=ptr_R;     //the right of ptr is ptr_R
  ptr_R->left=NULL;     //leaf node so no left or right node
  ptr_R->right=NULL;
  ptr_R->module=&modules[n];  //set modules into ptr_R modules
  if(n<num_modules){
