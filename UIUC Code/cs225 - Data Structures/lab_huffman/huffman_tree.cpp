                            ...
                            queue<TreeNode*>& mergeQueue)
{
  TreeNode * smallest=NULL;
    ...
    if(!(mergeQueue.empty()) && !(singleQueue.empty())){
      TreeNode * singleTemp = singleQueue.front();
      TreeNode * mergeTemp = mergeQueue.front();
      if(mergeTemp->freq.getFrequency() > singleTemp->freq.getFrequency()){
        smallest = singleQueue.front();
        singleQueue.pop();
        return smallest;
      }
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while(singleQueue.size()+mergeQueue.size() != 1){
      TreeNode* node1;
      TreeNode* node2; // node1 and node2 smallest(node1 is smallest);
      node1 = removeSmallest(singleQueue,mergeQueue);
      node2 = removeSmallest(singleQueue,mergeQueue);
      TreeNode* temp;
      int temp2 = node1->freq.getFrequency();
      int temp3 = node2->freq.getFrequency();
      temp = new TreeNode(temp2 + temp3);
      temp->left = node1;
      temp->right = node2;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    if(bfile.hasBits()){
      if(bfile.getNextBit() != true){
        ...
        node1->left = readTree(bfile);
        node1->right = readTree(bfile);
        return node1;
        TreeNode * node;
