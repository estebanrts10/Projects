...
{
    Node* subRoot = root;
    while(subRoot != NULL ||(subRoot->left != NULL || subRoot->right != NULL)){
      if(subRoot->left == NULL){
        break;
      }
      Node* left = subRoot->left;
      Node* right = subRoot->right;
      if(left != NULL && left > subRoot){
        return false;
      }
      if(right != NULL && right < subRoot){
        return false;
      }
    }
    while(subRoot != NULL ||(subRoot->left != NULL || subRoot->right != NULL)){
...
bool BinaryTree<T>::isOrderedRecursive(const Node* subRoot) const
{
  bool isOrder;
    ...
    if(subRoot->left!=NULL){
    isOrder = isOrderedRecursive(subRoot->left);
    }
    if(subRoot->right!=NULL){
    isOrder = isOrderedRecursive(subRoot->right);
}
    return isOrder;
