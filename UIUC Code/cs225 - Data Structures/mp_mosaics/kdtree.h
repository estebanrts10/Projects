...
class KDTree
{
  ...
  private:
     ...
     */
     ...
     void treeBuild(KDTreeNode *& nPoint, int left, int right);
     void buildHelper(int left, int right, int dimen);
     Point<Dim> findNearestNeighborHelper(const Point<Dim>& query,
          Point<Dim>& nPoint, KDTreeNode* subroot,
          double & dist, int dimen, int left, int right) const;
    double distance(const Point<Dim>& query, Point<Dim>& nPoint) const;
};
...
/*  if(left == right){
    if(checkBool == true){
      ...
      dist = 0;
      for(int i = 0; i < Dim; i++){
        dist = sqrt(dist + (query[i]-nPoint[i])*(query[i]-nPoint[i]));
      }
    }
    else{
      if(shouldReplace(query, nPoint, points[left])){
        nPoint = points[left];
