 ...
 */
 ...
 size_t insertion_index(const std::vector<T>& elements, const C& val, int left, int right){
   T eLeft = elements[left];
   T eRight = elements[right];
   T center = elements[(left+right) / 2];
   if(left==right-1 && val > eLeft && val < eRight){
     return right;
   }
  if(val == center){
    return ((left+right)/2);
  ...
  if(val>center){
    return insertion_index(elements, val, (left+right)/2, right);
  }
  if(val<center){
    return insertion_index(elements, val, left, (left+right)/2);
  }
  return 0;
 }
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    if(val>elements[elemSize-1]){
      return elemSize;
    }
    if(val == elements[elemSize-1]){
      return elemSize-1;
    }
    return insertion_index(elements,val,0,elemSize-1);
